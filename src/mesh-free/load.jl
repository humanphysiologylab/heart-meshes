...
filename_tetra = joinpath(folder, "M13/M13_IRC_tetra.int32")
tetra = read_binary(filename_tetra, Int32, (4, :))
tetra = permutedims(tetra, (2, 1))
tetra .+= 1

filename_points = joinpath(folder, "M13/M13_IRC_3Dpoints.float32")
points = read_binary(filename_points, Float32, (3, :))
points = permutedims(points, (2, 1))

filename_I_tetra = joinpath(folder, "M13/I_tetra.int32")
I_tetra = read_binary(filename_I_tetra, Int32)
filename_J_tetra = joinpath(folder, "M13/J_tetra.int32")
...
filename_times = joinpath(
    folder,
    "activation/data-light/M13/G1/S13/times.float32"
)
times = read_binary(filename_times, Float32)

filename_starts = joinpath(
    folder,
    "activation/data-light/M13/G1/S13/indices_start.int32"
)
starts = read_binary(filename_starts, Int32)

A_vertices = load_adj_matrix(joinpath(folder, "M13/adj_matrix"), false)
